...
n1 <- 2000  #number of observations (training and test set combined)
sig1 <- 5  #error standard deviation

x1 <- runif(n1, 0, 10) #generate x variable Unif(0,10) distribution
e1 <- rnorm(n1, 0, sig1) #generate error term 
y1 <- 2*x1+3 + e1  #generate response values 

data1 <- data.frame(x1, y1) #combine explanatory and response varables into data frame
trainOne <- data1[1:(n1/2), ] #1st half of observations as training data
test1 <- data1[(n1/2+1):n1, ] #2nd half of observations as test data
```
...
for (k in 1:n/2) {
  if (PI1[k,2] <= test1[k,2]  & test1[k,2] <= PI1[k,3]) {ON1[k] <- "Y"}
  else {ON1[k] <- "N"}
}
...
#Simulate
set.seed(06232019) #set seed
n3 <- 2000  #number of observations (trainig and test set combined)
...
e <- rnorm(n3, 0, sig) #generate error term 
...
trainThree <- data3[1:(n3/2), ] #use first half of observations as training data
test3 <- data3[(n3/2+1):n3, ] #use second half of observations as test data
```
