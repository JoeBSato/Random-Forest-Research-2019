...
y <- 2*x1+3 + e1  #generate response values 
...
resultat <- Results(traindata=trainOne, testdata=test1, nodesize = 10)
```
...
his1 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
tra1 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
...
x<- runif(n, 0, 10) #generate x variable Unif(0,10) distribution
e <- rnorm(n, 0, sig1) #generate error term 
y <- 2*x1+3 + e  #generate response values 
...
#rec1
rec1[,,t] <- Results(traindata=trainOne, testdata=test1, nodesize =  1)
#his1
his1[,,t] <- Results(traindata=trainOne, testdata=test1, nodesize =  5)
#tra1
tra1[,,t] <- Results(traindata=trainOne, testdata=test1, nodesize = 10)
}
```

```{r}
#Make the table by taking average over i=1:10
rec1Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    rec1Tab[k,j] <- mean(rec1[k,j,])
  }
}

his1Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    his1Tab[k,j] <- mean(his1[k,j,])
  }
}

tra1Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    tra1Tab[k,j] <- mean(tra1[k,j,])
  }
}
```
...
his2 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
tra2 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
...
data2 <- data.frame(x, y) #combine explanatory and response varables into data frame
trainTwo <- data2[1:(n/2), ] #1st half of observations as training data
test2 <- data2[(n/2+1):n, ] #2nd half of observations as test data

#rec2
rec2[,,t] <- Results(traindata=trainTwo, testdata=test2, nodesize =  1)
#his2
his2[,,t] <- Results(traindata=trainTwo, testdata=test2, nodesize =  5)
#tra2
tra2[,,t] <- Results(traindata=trainTwo, testdata=test2, nodesize = 10)
}
```

```{r}
#Make the table by taking average over i=1:10
rec2Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    rec2Tab[k,j] <- mean(rec2[k,j,])
  }
}

his2Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    his2Tab[k,j] <- mean(his2[k,j,])
  }
}

tra2Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for(k in 1:2) {
  for (j in 1:3) {
    tra2Tab[k,j] <- mean(tra2[k,j,])
  }
}
```
...
his3 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
tra3 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
...
data3 <- data.frame(X, y) #combine explanatory and response varables into data frame
trainThree <- data3[1:(n/2), ] #1st half of observations as training data
test3 <- data3[(n/2+1):n, ] #2nd half of observations as test data

#rec3
rec3[,,t] <- Results(traindata=trainThree, testdata=test3, nodesize =  1)
#his3
his3[,,t] <- Results(traindata=trainThree, testdata=test3, nodesize =  5)
#tra3
tra3[,,t] <- Results(traindata=trainThree, testdata=test3, nodesize = 10)
}
```

```{r}
#Make the table by taking average over i=1:10
rec3Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    rec3Tab[k,j] <- mean(rec3[k,j,])
  }
}

his3Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    his3Tab[k,j] <- mean(his3[k,j,])
  }
}

tra3Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    tra3Tab[k,j] <- mean(tra3[k,j,])
  }
}
...
```{r}
set.seed(06232019)#set seed
 #number of observations (training and test set combined)
sig1 <- 5  #error standard deviation
i <- 10

#Make a result storing array: MSE, Avg. Width, # in PI
rec4 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
his4 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
tra4 <- array(NA, dim=c(2,3, i), dimnames=list(c("LR","RF"),c("MSE","avg.width","# in PI"),c(1:i)))
...
data4 <- data.frame(X, y) #combine explanatory and response varables into data frame
trainFour <- data4[1:(n/2), ] #1st half of observations as training data
test4 <- data4[(n/2+1):n, ] #2nd half of observations as test data

#rec4
rec4[,,t] <- Results(traindata=trainFour, testdata=test4, nodesize =  1)
#his4
his4[,,t] <- Results(traindata=trainFour, testdata=test4, nodesize =  5)
#tra4
tra4[,,t] <- Results(traindata=trainFour, testdata=test4, nodesize = 10)
}
```

```{r}
#Make the table by taking average over i=1:10
rec4Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    rec4Tab[k,j] <- mean(rec4[k,j,])
  }
}

his4Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    his4Tab[k,j] <- mean(his4[k,j,])
  }
}

tra4Tab <- matrix(NA, nrow=2, ncol=3, dimnames = list(c("LR","RF"),c("Avg.MSE","Avg.Wid","Avg.CR")))

for (k in 1:2) {
  for (j in 1:3) {
    tra4Tab[k,j] <- mean(tra4[k,j,])
  }
}
...
```{r}
Fin <- list(rec=list(one=rec1Tab,two=rec2Tab, three=rec3Tab, four=rec4Tab), his=list(one=his1Tab, two=his2Tab, three=his3Tab,four=his4Tab), tra=list(one=tra1Tab, two=tra2Tab, three=tra3Tab, four=tra4Tab))
```
